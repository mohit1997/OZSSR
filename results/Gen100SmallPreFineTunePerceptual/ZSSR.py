...
from utils import *
from simplenet import simpleResNetBNP, SmallNet, simpleNet, simpleResNet, simpleResNetBN, ResBuild, DenoiseSuperRes
from laploss import *
import sys
...
class ZSSR:
    ...
    def __init__(self, input_img, conf=Config(), ground_truth=None, kernels=None):
        ...
        print(self.Y)
        bands = 0
        # self.model = simpleNet(self.Y, inp_channels=3 + 4*bands)
        # self.model = simpleResNet(self.Y, inp_channels=3 + 4*bands)
        # self.model = simpleResNetBN(self.Y, inp_channels=3 + 4*bands)
        self.model = simpleResNetBNP(self.Y, inp_channels=3 + 4*bands)
        # self.model = SmallNet(self.Y, inp_channels=3 + 4*bands)
        # self.model = UNet(in_channels=3+4*bands, out_channels=3)
        # self.model = ResBuild(in_channels=256, out_channels=3)
        # self.model = DenoiseSuperRes(in_channels=256, out_channels=3)
        self.model_path = 'best_perceptual.pth'
        if path.exists(self.model_path):
            mod = torch.load(self.model_path)
            self.model.load_state_dict(mod)
        ...
        xv = np.pi * np.expand_dims(xv, axis=-1)
        yv = np.expand_dims(yv, axis=-1)
        encx = [(np.cos(2**i*xv)+1)/2. for i in range(bands)] + [(np.sin(2**i*xv)+1)/2. for i in range(bands)]
        ency = [(np.cos(2**i*yv)+1)/2. for i in range(bands)] + [(np.sin(2**i*yv)+1)/2. for i in range(bands)]
        lis = [self.input] + encx + ency
        new_input = np.concatenate(lis, axis=2)
    ...
    def run(self):
        ...
        return post_processed_output, self.gt
    ...
    def forward_backward_pass(self, lr_son, hr_father,criterion,optimizer):
        ...
        if self.Y == True:
            lr_son_input = torch.Tensor(lr_son).unsqueeze_(1)
            hr_father = torch.Tensor(hr_father).unsqueeze_(1)
        else:
            # lr_son_input = torch.Tensor(lr_son).permute(2,0,1).unsqueeze_(0)
            # hr_father = torch.Tensor(hr_father).permute(2,0,1).unsqueeze_(0)
            lr_son_input = torch.Tensor(lr_son).permute(0,3,1,2)
            hr_father = torch.Tensor(hr_father).permute(0,3,1,2)
            # print(lr_son_input.type(), hr_father.type())
    ...
    def forward_pass(self, lr_son, hr_father_shape=None):
        ...
        # interpolated_lr_son = transform.resize(lr_son, (hr_father_shape[0], hr_father_shape[1]), order=3)
        if self.Y == True:
    ...
    def quick_test(self):
        ...
        # img = transform.resize(self.input, (100, 100), order=3)
        # print(out.shape)
        # shape = (self.input.shape[0]*2, self.input.shape[1]*2)
        self.sr = self.forward_pass(self.input)
        self.mse.append(np.mean((self.gt - self.sr)**2)
                    if self.gt is not None else None)

        # if self.conf.display_test_results:
        #     print('iteration: ', self.iter, ', true mse {:.6f}'.format(round(self.mse[-1], 4)))
        ...
        self.mse_rec.append(np.mean(np.ndarray.flatten(np.square(self.raw_input - self.reconstruct_output[:self.input.shape[0], :self.input.shape[1]]))))
        ...
        self.interp_mse = (self.interp_mse + [np.mean(np.ndarray.flatten(np.square(self.gt - interp_sr[:, :, :3])))]
                           if self.gt is not None else None)
        ...
        if self.conf.display_test_results:
            print('iteration: ', self.iter, 'reconstruct mse: {:.5f}'.format(self.mse_rec[-1]), ', true mse:', (round(self.mse[-1], 5)
    ...
    def train(self):
        ...
        loss_fn = Percpmodels.PerceptualLoss(model='net-lin', net='vgg', use_gpu=self.cuda)
        optimizer = torch.optim.Adam(self.model.parameters(),lr = self.learning_rate)
        # sch = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.1, patience=100, threshold=0.01, verbose=True, min_lr=1e-4)
        sch = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[250,500], gamma=0.1)
        # main training loop
        lis_father = []
        lis_son = []
        print(path.exists(self.path+"_father.npy"))
        if path.exists(self.path+"_father.npy"):
            father_data = np.load(self.path+"_father.npy")
            son_data = np.load(self.path+"_son.npy")
            print(father_data.dtype)
            print(son_data.dtype)
            print("Data Loaded")
        else:
            train_transform = transforms.Compose([
            transforms.ToPILImage(),
            transforms.RandomAffine(20, translate=(0.2, 0.2), scale=(0.5, 1.5), shear=20, resample=3, fillcolor=0),
            transforms.RandomHorizontalFlip(p=0.5),
            # transforms.RandomRotation(20, fill=0),
            # transforms.RandomPerspective(distortion_scale=0.5, p=0.5, interpolation=3, fill=0),
            transforms.RandomApply([transforms.RandomResizedCrop((128, 128), scale=(0.7, 1.0),ratio=(1., 1.), interpolation=3)], p=0.5),
            transforms.Resize((128, 128), interpolation=3),
            # transforms.ToTensor(),
            ])
            inp = np.uint8(self.input * 255)
            out = train_transform(inp)
            # sys.exit()
            for i in range(3000):
                ...
                # o = train_transform(inp)
                # self.hr_father = np.clip(np.array(o)/255., 0, 1.)
                # print(self.hr_father.shape)

                # Get lr-son from hr-father

                self.lr_son =  self.father_to_son(self.hr_father)
                interpolated_lr_son = imresize(self.lr_son, self.sf, self.hr_father.shape, self.conf.upscale_method)
                # interpolated_lr_son = transform.resize(self.lr_son, (self.hr_father.shape[0], self.hr_father.shape[1]), order=3)
                lis_father.append(self.hr_father)
                lis_son.append(interpolated_lr_son)
            father_data = np.stack(lis_father, axis=0).astype('float32')
            son_data = np.stack(lis_son, axis=0).astype('float32')
            # np.save(self.path+"_father", father_data)
            # np.save(self.path+"_son", son_data)
            print("Data Created")
        
        train_transform = transforms.Compose([
            # transforms.ToPILImage(),
            # transforms.RandomHorizontalFlip(p=0.5),
            # transforms.RandomApply([transforms.RandomResizedCrop(28, scale=(0.8, 1.0),ratio=(1., 1.))], p=0.5),
            transforms.ToTensor(),
            ])
        
        dataset = DataLoaderCustom(son_data, father_data, train_transform)
        trainloader = torch.utils.data.DataLoader(dataset, batch_size=8, shuffle=True, num_workers=4, drop_last=False)  #DO NOT CHANGE
        use_cuda = self.cuda and torch.cuda.is_available()

        device = torch.device("cuda" if use_cuda else "cpu")
        for epoch in range(2):
            # for self.iter in range(self.conf.max_iters):
            for self.iter, sample in enumerate(trainloader):
                #should convert input and output to torch tensor
                # ind = np.random.randint(0, 3000-17, 16)
                # ind = np.arange(self.iter*16, (self.iter+1)*16)
                # father_batch = father_data[ind]
                # son_batch = son_data[ind]
                # print(sample['x'].dtype, print(son_data.dtype))
                self.model.train(True) # only for pretrained resnet

                lr_son_input = sample['x'].to(device)
                hr_father = sample['y'].to(device)
              
                train_output = self.model(lr_son_input)
                ploss = loss_fn.forward(hr_father[:, :3], train_output, normalize=True)
                ploss = 2.*torch.mean(ploss)
                # loss = 0*criterion(hr_father[:, :3], train_output) + ploss
                loss = ploss
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                ...
                # _ = self.forward_backward_pass(son_batch, father_batch, criterion, optimizer)
                # Display info and save weights
                if not self.iter % self.conf.display_every:
                    print('sf:', self.sf*self.base_sf, ', iteration: ', self.iter, ', loss: {:.4f}'.format(loss.item()))
