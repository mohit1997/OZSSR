...
class DataLoaderCustom(Dataset):
  def __init__(self, in_images, out_images, transform=None):
    """
    Args:
  ...
  def __getitem__(self, idx):
    sample = {}
    sample['x'] = self.in_images[idx]
    sample['y'] = self.out_images[idx]
...
def random_augment(ims,
    ...
    for i in range(0, im.shape[2], 3):
        temp = np.clip(warpPerspective(im[:,:, i:i+3], transform_mat, (crop_size, crop_size), flags=INTER_CUBIC), 0, 1)
        # lis.append(transform.resize(temp, (crop_size, crop_size)))
        lis.append(temp)
