...
def main(conf_name, gpu):
    ...
    for file_ind, input_file in enumerate(files):
        ...
        # The other option is just to run sequentially on a chosen GPU.
            ...
            mse, psnr, ssim = run_ZSSR_single_input.main(input_file, ground_truth_file, kernel_files_str, gpu, conf_name, res_dir)
            logger['mse'].append(mse)
            logger['psnr'].append(psnr)
            logger['ssim'].append(ssim)

        print("Mean Test Error {:.4f} PSNR {:.3f} SSIM {:.3f}".format(np.mean(logger['mse']),
         np.mean(logger['psnr']),
         np.mean(logger['ssim']) ))
        
        f = open('%s/log_zssr.pkl' % (conf.result_path), "wb")
